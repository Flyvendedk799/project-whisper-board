...
} from "@/components/ticket-attachments-field";
    ...
    <>
      <PageHeader
        ...
        description={
          (
            <>
              Reported by {t.reporter?.full_name ?? t.reporter?.email ?? "—"} · in{" "}
              <Link
        ...
        action={
          <Button variant="ghost" asChild>
            <Link to="/app/projects/$projectId" params={{ projectId: t.project.id }}>
        ...
        <div className="space-y-6 min-w-0">
          ...
          <div className="space-y-4">
            ...
            <CommentBox
              ...
              authorId={user!.id}
              isAdmin={!!isAdmin}
              onSent={() => qc.invalidateQueries({ queryKey: ["comments", ticketId] })}
            />
          </div>
        </div>
        ...
        <aside className="space-y-4">
          {isAdmin && (
            <Card className="p-4 space-y-3">
              ...
              <AISummaryButton
                ticketId={ticketId}
                onDone={() => qc.invalidateQueries({ queryKey: ["ticket", ticketId] })}
              />
              <AutoTriageButton
                ticketId={ticketId}
                onDone={() => qc.invalidateQueries({ queryKey: ["ticket", ticketId] })}
              ...
              {t.ai_suggested_type && (
                <div className="text-xs text-muted-foreground">
                  AI suggests: <b>{t.ai_suggested_type}</b> · <b>{t.ai_suggested_priority}</b>
                </div>
              )}
            </Card>
          )}
  ...
  async function send() {
    ...
    if (body.trim()) {
      const { error } = await supabase
        .from("ticket_comments")
        .insert({ ticket_id: ticketId, author_id: authorId, body, is_internal: internal });
      if (error) {
        setBusy(false);
    ...
    <Card className="p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-sm">
          ...
          {isAdmin && (
            <Button type="button" variant="ghost" size="sm" onClick={aiDraft}>
...
function AutoTriageButton({ ticketId, onDone }: { ticketId: string; onDone: () => void }) {
  ...
  return (
    <Button
      ...
      onClick={async () => {
        ...
        try {
          ...
          toast.success(`Suggests: ${r.type} / ${r.priority}`);
          onDone();
        } catch (e: any) {
          toast.error(e.message);
        } finally {
    ...
    >
      ...
      {busy ? "Triaging…" : "Auto-triage"}
    </Button>
  );
}

function ScreenshotAIButton({
  ticketId,
  attachments,
  onDone,
}: {
  ...
  return (
    <Button
      ...
      onClick={async () => {
        setBusy(true);
        try {
          await fn({ data: { ticketId, attachmentId: firstImage.id } });
...
function AttachmentTile({ att }: { att: any }) {
  ...
  useEffect(() => {
    sign({ data: { bucket: att.storage_bucket, path: att.storage_path } })
      .then((r) => setUrl(r.url))
      .catch(() => {});
  }, [att.id]);
