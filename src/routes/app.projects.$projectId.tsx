        ...
        <Tabs defaultValue="tickets">
          ...
          <TabsContent value="tickets" className="mt-6">
            {(tickets.data?.length ?? 0) === 0 ? (
              <Card className="p-10 text-center text-sm text-muted-foreground">
                No tickets yet. Create one to start the conversation.
              </Card>
            ) : (
              <Card className="divide-y">
                {tickets.data!.map((t) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground uppercase tracking-wide w-16">
                      {t.type}
                    </span>
          ...
          <TabsContent value="milestones" className="mt-6 space-y-3">
            {isAdmin && (
              <NewMilestoneRow
                projectId={projectId}
                onCreated={() => qc.invalidateQueries({ queryKey: ["milestones", projectId] })}
              />
            )}
            ...
            <Card className="divide-y">
              ...
              {(members.data?.length ?? 0) === 0 && (
                <div className="p-6 text-sm text-muted-foreground text-center">
                  No members yet. {isAdmin && "Invite a client above."}
                </div>
              )}
            </Card>
          </TabsContent>
      ...
      <DialogContent>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            They'll get an email with a sign-in link and immediate access to this project.
          </p>
          <DialogFooter>
...
function NewTicketButton({
  ...
  const [type, setType] = useState<"bug" | "feature" | "question" | "feedback" | "change_request">(
    "bug",
  );
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Summarize the bug, request, or question"
            />
          </div>
          <div className="space-y-2">
