...
export const inviteClient = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Ensure role
    await a.from("user_roles").upsert({ user_id: userId, role: "client" }, { onConflict: "user_id,role" });
...
export const signedAttachmentUrl = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) =>
    z.object({ bucket: z.enum(["attachments", "recordings"]), path: z.string().min(1).max(500) }).parse(input),
  )
  .handler(async ({ data, context }) => {
    ...
    const a = admin();
    const { data: signed, error } = await a.storage.from(data.bucket).createSignedUrl(data.path, 60 * 10);
