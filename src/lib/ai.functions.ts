...
export const autoTriageTicket = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: t } = await supabase.from("tickets").select("title,description,type,priority").eq("id", data.ticketId).maybeSingle();
    if (!t) throw new Error("Not found");
    ...
    await supabase
      .from("tickets")
      .update({ ai_suggested_type: type, ai_suggested_priority: priority, ai_summary: parsed.reasoning ?? null })
      .eq("id", data.ticketId);
...
export const analyzeScreenshot = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => z.object({ ticketId: z.string().uuid(), attachmentId: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: att } = await supabase.from("ticket_attachments").select("storage_bucket,storage_path,mime_type").eq("id", data.attachmentId).maybeSingle();
    if (!att) throw new Error("Attachment not found");
    if (!att.mime_type?.startsWith("image/")) throw new Error("Not an image");
    const { data: signed } = await supabase.storage.from(att.storage_bucket).createSignedUrl(att.storage_path, 60 * 5);
    ...
    const analysis = await callAI([
      { role: "system", content: "You analyze a screenshot from a bug report. Describe in 2-4 sentences what's visible, any errors/UI issues, and what the user might be trying to do." },
      { role: "user", content: [{ type: "text", text: "Analyze this screenshot:" }, { type: "image_url", image_url: { url: dataUrl } }] },
    ]);
    await supabase.from("tickets").update({ ai_screenshot_analysis: analysis }).eq("id", data.ticketId);
    return { analysis };
  });
...
export const meetingNotesToTickets = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: m } = await supabase.from("meetings").select("project_id,title,notes").eq("id", data.meetingId).maybeSingle();
    ...
    for (const it of items) {
      const { data: ticket } = await supabase
        ...
        .insert({
          ...
          type: ["bug", "feature", "question", "change_request"].includes(it.type) ? it.type : "feature",
    ...
    await supabase.from("meetings").update({ ai_summary: parsed.summary ?? null }).eq("id", data.meetingId);
