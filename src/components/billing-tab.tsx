    ...
    <div className="space-y-8">
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {isAdmin && (
            <NewQuoteButton projectId={projectId} currency={currency} onCreated={refresh} />
          )}
        </div>
        {(quotes.data?.length ?? 0) === 0 ? (
          ...
          <div className="space-y-2">
            {quotes.data!.map((q) => (
              <QuoteCard key={q.id} quote={q} canEdit={!!isAdmin} onChanged={refresh} />
            ))}
          </div>
        )}
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {isAdmin && (
            <NewInvoiceButton projectId={projectId} currency={currency} onCreated={refresh} />
          )}
        </div>
            ...
            {invoices.data!.map((i) => (
              <div key={i.id} className="flex items-center gap-3 p-4">
                ...
                {isAdmin && i.status !== "paid" && (
                  <MarkPaidButton invoiceId={i.id} onDone={refresh} />
                )}
              </div>
            ))}
          </Card>
...
function NewQuoteButton({
  ...
  const [lines, setLines] = useState<Line[]>([
    { description: "", unit_price_cents: 0, quantity: 1 },
  ]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setTitle("");
      setNotes("");
      setLines([{ description: "", unit_price_cents: 0, quantity: 1 }]);
      onCreated();
    } catch (e: any) {
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setLines((arr) =>
                      arr.map((x, j) =>
                        j === i
                          ? { ...x, unit_price_cents: Math.round(Number(e.target.value) * 100) }
                          : x,
                      ),
                    )
                  }
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                setLines((arr) => [...arr, { description: "", unit_price_cents: 0, quantity: 1 }])
              }
            >
          ...
          <div className="text-right text-sm">
            ...
            <span className="font-medium">{formatCents(subtotal, currency)}</span>
          </div>
          <DialogFooter>
...
function QuoteCard({
  ...
  async function doSend() {
    setBusy(true);
    try {
      await send({ data: { quoteId: quote.id } });
  ...
  async function doRespond(accept: boolean) {
    setBusy(true);
    try {
      await respond({ data: { quoteId: quote.id, accept } });
    ...
    <Card className="p-5 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <StatusPill tone={tone}>{quote.status}</StatusPill>
          </div>
          ...
          {quote.quote_line_items
            ...
            .map((l: any) => (
              <div key={l.id} className="flex justify-between">
                ...
                <span>
                  {formatCents(Math.round(l.unit_price_cents * l.quantity), quote.currency)}
                </span>
              </div>
            ))}
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {!canEdit && quote.status === "sent" && (
          <>
            <Button size="sm" onClick={() => doRespond(true)} disabled={busy}>
              Accept
            </Button>
            <Button size="sm" variant="outline" onClick={() => doRespond(false)} disabled={busy}>
              Decline
            </Button>
          </>
        )}
