...
export function TicketAttachmentsField({
  ...
  function addFiles(
    files: FileList | File[],
    bucket: "attachments" | "recordings" = "attachments",
  ) {
  ...
  async function startRecording() {
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File(
          [blob],
          `recording-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.webm`,
          { type: "video/webm" },
        );
      ...
      stream
        .getVideoTracks()[0]
        .addEventListener("ended", () => recorder.state !== "inactive" && recorder.stop());
      recorder.start();
