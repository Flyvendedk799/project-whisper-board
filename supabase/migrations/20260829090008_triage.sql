-- Everything the triage queue sorts, filters and saves.
...
create table public.saved_views (
  ...
  -- The route's validated search params, stored verbatim. Loading a view is
  -- navigate({ search: filters }); there is no separate serialisation format.
...
values ('urgent', 60, 480),        -- reply within the hour, fixed inside a day
       ('high', 240, 2880),        -- half a day to reply, two days to resolve
       ('medium', 1440, 10080),    -- a day to reply, a week to resolve
       ('low', 4320, 43200);       -- three days to reply, a month to resolve

-- SLA due date is computed in SQL so no two clients disagree about it.
...
-- Resolution and reopen bookkeeping.
...
-- First response means the first time somebody who is not the reporter replies
-- where the reporter can see it. An internal note is not a response.
...
-- Backfill so existing tickets are not all reported as breached.
update public.tickets t
